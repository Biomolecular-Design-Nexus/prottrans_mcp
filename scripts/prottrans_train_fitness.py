#!/usr/bin/env python3
"""
ProtTrans-based Protein Fitness Prediction Training Script

This script trains regression models on ProtTrans embeddings for protein fitness prediction.
It performs 5-fold cross-validation for evaluation and then trains a final model on all data.

Usage:
    python prottrans_train_fitness.py -i <data_dir> -o <output_dir> -b <model_name>
...
def load_data(data_dir, backbone_model="ProtT5-XL", target_col="log_fitness"):
    """
    Load ProtTrans embedding data and target values.
    ...
    # 1. data_dir/{backbone_model}/{backbone_model}.npy
    # 2. data_file.{backbone_model}.npy
    embd_save_path = f"{data_file}.{backbone_model}.npy"
    ...
    if os.path.exists(embd_save_path):
        ...
        for idx, seq in enumerate(df_data["seq"]):
            if seq in seq2embd:
                ...
                raise ValueError(
                    f"Could not find embeddings for sequence {seq} at index {idx} in data.csv"
                )
...
def perform_cross_validation(Xs, Ys, args):
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(Xs)):
        ...
        logger.info(
            f"Train Spearman: {spearman_r_train:.4f}, Test Spearman: {spearman_r_test:.4f}"
        )

        # Save fold model and predictions
        joblib.dump(
            head_model,
            os.path.join(fold_output_dir, f"head_model_{args.head_model}.joblib"),
        )
...
def train_final_model(Xs, Ys, args):
    ...
    Xs_pca, _, pca_model = apply_pca(
        Xs, n_components=args.n_components, output_dir=final_output_dir, save_model=True
    )
    ...
    model_path = os.path.join(
        final_output_dir, f"head_model_{args.head_model}.joblib"
    )
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="ProtTrans-based protein fitness prediction with 5-fold CV and final model training",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Train with default settings (ProtT5-XL, SVR, PCA 60)
  python prottrans_train_fitness.py -i data/proteins -o results

  # Use ProtAlbert model
  python prottrans_train_fitness.py -i data/proteins -o results -b ProtAlbert

  # Use different head model
  python prottrans_train_fitness.py -i data/proteins -o results -m random_forest

  # Custom PCA components
  python prottrans_train_fitness.py -i data/proteins -o results -n 100

Available ProtTrans models:
...
Available head models:
  ...
  - xgboost
    ...
    parser.add_argument(
        ...
        help="ProtTrans backbone model (default: ProtT5-XL)",
    )
    parser.add_argument(
        "-m",
        "--head_model",
...
def main():
    ...
    # Print configuration
    logger.info("=" * 80)
    logger.info("ProtTrans Fitness Prediction Training")
    logger.info("=" * 80)
    ...
    cv_results_path = os.path.join(
        args.output_dir, f"{args.backbone_model}_{args.head_model}_cv_results.csv"
    )
